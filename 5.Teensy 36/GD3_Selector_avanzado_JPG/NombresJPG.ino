...
{
  GD.ColorA(Colorlista); // GD.ColorRGB(0x000000);
  for (int j = 0; j < num_jpgs; j++) {
    int y = 26 * j - yb;
    if ((-50 <= y) && (y <= 450)) {
      colorselecto= j;  if (colorselecto==NombreJPG){GD.ColorRGB(0xff0000);}else{GD.ColorRGB(0x151515);}   //Resalta el nombre de la imagen en uso en el menu principal, en este caso: LeenombreJPG()
